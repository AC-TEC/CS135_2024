...
Task E: write a program that scales the original image to 200% of its size
*/
...
int main() {
	...
	for(int row = 0; row < h ; row++){
		for(int col = 0; col < w; col++){
            ...
            new_img[2*row][2*col] = pixel;
            new_img[2*row][2*col+1] = pixel;
            new_img[2*row+1][2*col] = pixel;
            new_img[2*row+1][2*col+1] = pixel;
        }
	}

	//save this new image to file "outImage.pgm"
	writeImage(new_img, new_h, new_w);
