...
Task F:  write program to pixelate input image
*/
...
int main() {
    ...
    // Loop through non-overlapping 2x2 windows
    for (int row = 0; row < h; row += 2) {
        for (int col = 0; col < w; col += 2) {
            // Calculate average value of pixels in window
            int sum = 0;
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    sum += img[row+i][col+j];
            ...
            // Set all pixels in window to average value
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    img[row+i][col+j] = avg;
                }
